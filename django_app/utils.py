...
print(__file__)
BASEDIR_CSV_SP: str = os.path.join('..', 'parameter_sets')
BASEDIR_CSV_ECM: str = os.path.join('..', 'parameter_sets_ecm')
DICT_FILENAMES_ECM: dict = {'param.csv': 'bc'}
DICT_FILENAMES_SP: dict = {'param_battery-cell.csv': 'bc',
        ...
        for i, row in enumerate(csv_reader):
            if i != 0:
                if len(row[0].split(' [')) >= 2:
                    param_name: str = row[0].split(' [')[0] + f"_{filetype} [" + row[0].split(' [')[
                        -1]  # the string split
...
def get_csv_filepath(parameter_name: str, csv_filename: str, param_type: str) -> str:
    """
    Returns the filepath to the csv file given the parameter name and csv file name.
    """
    if param_type == "ECM":
        basedir: str = BASEDIR_CSV_ECM
    elif param_type == "SP":
        basedir: str = BASEDIR_CSV_SP
    else:
        raise ValueError(f'Invalid param_type: {param_type}. Available options are ECM, SP')
    return os.path.join(basedir, parameter_name, csv_filename)


def create_dict_parameterset(param_type: str) -> dict:
    """
    ...
    dict_final: dict = {}
    # dict_filename: dict = {}  # dict that intends to store the filenames containing parameters
    if param_type == 'ECM':
        dict_filename: dict = DICT_FILENAMES_ECM
        lst_parameterset_names: list = SPPy.battery_components.parameter_set_manager.ECMParameterSets.lst_parameter_names()
    elif param_type == 'SP':
        dict_filename: dict = DICT_FILENAMES_SP
        lst_parameterset_names: list = SPPy.battery_components.parameter_set_manager.ParameterSets.list_parameters_sets()
    else:
        raise ValueError(f"Invalid param_type {param_type}: valid options are 'ECM', 'SP'")
    for parameterset_name in lst_parameterset_names:
        ...
        for filename in dict_filename.keys():  # for each above iteration iterate over all files
            filepath: str = get_csv_filepath(parameter_name=parameterset_name,
                                             csv_filename=filename,
                                             param_type=param_type)
            filetype: str = dict_filename[filename]
            try:
...
def save_json_file(filepath_to_json: str, param_type: str = 'SP') -> None:
    with open(filepath_to_json, 'w', encoding='utf-8') as json_file:
        json_file.write(json.dumps(create_dict_parameterset(param_type=param_type), indent=4))


# save_json_file('static/parameter_sets.json')
save_json_file('static/parameter_sets_ecm.json', param_type='ECM')
