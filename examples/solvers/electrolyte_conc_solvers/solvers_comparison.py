...
from SPPy.solvers.electrolyte_conc import ElectrolyteConcVolAvgSolver, ElectrolyteConcFVMSolver
...
co_ords: ElectrolyteFVMCoordinates = ElectrolyteFVMCoordinates(L_n=L_n, L_s=L_s, L_p=L_p)
conc_fvm_solver: ElectrolyteConcFVMSolver = ElectrolyteConcFVMSolver(fvm_co_ords=co_ords, transference=t_c,
...
conc_avg_solver: ElectrolyteConcVolAvgSolver = ElectrolyteConcVolAvgSolver(L_n=L_n, L_s=L_s, L_p=L_p,
...
j_p_avg: float = SPMe.molar_flux_electrode(I=-1.656, S=1.1167, electrode_type='p')  # [mol/m2/s]
j_n_avg: float = SPMe.molar_flux_electrode(I=-1.656, S=0.7824, electrode_type='n')  # [mol/m2/s]

j_p: np.ndarray = SPMe.molar_flux_electrode(I=-1.656, S=1.1167, electrode_type='p') * np.ones(len(co_ords.array_x_p))
# [mol/m2/s]
j_sep: np.ndarray = np.zeros(len(co_ords.array_x_s))  # [mol/m2/s]
j_n: np.ndarray = SPMe.molar_flux_electrode(I=-1.656, S=0.7824, electrode_type='n') * np.ones(len(co_ords.array_x_n))
# [mol/m2/s]
...
for simulation_index in range(t_iterations):
    conc_avg_solver.solve(t_prev=t_prev, avg_j_p=j_p_avg, avg_j_n=j_n_avg, dt=dt)
...
plt.title(f"Electrolyte Conc. [mol/m3] after {t_iterations * dt} s of discharge")
